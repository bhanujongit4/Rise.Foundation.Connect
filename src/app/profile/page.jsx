...
import { UserCircleIcon, BriefcaseIcon, PencilIcon, TrashIcon } from '@heroicons/react/24/solid';
...
const ProfilePage = () => {
  ...
  const handleUserUpdate = async () => {
    try {
      ...
      if (newProfilePicture) {
        const downloadURL = await handleProfilePictureUpload();
        if (downloadURL) {
          updatedUser.profilePictureUrl = downloadURL;
        }
      }
      await updateDoc(doc(db, 'users', session.user.id), updatedUser);
      setUser(updatedUser);
      setIsEditing(false);
      setNewProfilePicture(null);
    } catch (error) {
          ...
          <div className="w-32 h-32 mb-4 md:mb-0 md:mr-6">
            {isEditing ? (
              <div className="relative">
                {newProfilePicture ? (
                  <img src={URL.createObjectURL(newProfilePicture)} alt="New profile" className="w-full h-full rounded-full object-cover" />
